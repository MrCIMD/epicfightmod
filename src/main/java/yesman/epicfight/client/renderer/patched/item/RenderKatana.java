...
public class RenderKatana extends RenderItemBase {
	...
	public void renderItemInHand(ItemStack stack, LivingEntityPatch<?> entitypatch, InteractionHand hand, MultiBufferSource buffer, PoseStack poseStack, int packedLight) {
		...
		modelMatrix.mulFront(entitypatch.getEntityModel(Models.LOGICAL_CLIENT).getArmature().searchJointByName("Tool_R").getAnimatedTransform());
		...
		modelMatrix.mulFront(entitypatch.getEntityModel(Models.LOGICAL_CLIENT).getArmature().searchJointByName("Tool_L").getAnimatedTransform());
